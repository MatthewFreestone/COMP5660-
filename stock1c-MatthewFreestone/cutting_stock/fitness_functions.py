...
def multiobjective_fitness_function(solution, shapes, bounds, failure_fitness, **kwargs):
    ...
    if impl.count_overlaps(cells) or impl.count_out_of_bounds(cells, bounds):
        ...
        x_available = bounds[0][1] - bounds[0][0]
        y_available = bounds[1][1] - bounds[1][0]
        x_fitness = x_available - impl.get_extent(cells, False)
        y_fitness = y_available - impl.get_extent(cells, True)
