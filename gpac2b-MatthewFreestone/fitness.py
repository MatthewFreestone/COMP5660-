...
Returns Pac-Man score from a full game as well as the game log.
'''
def play_GPac_multicontroller(pac_controllers, ghost_controllers=None, game_map=None, **kwargs):
    game_map = parse_map(game_map)
    game = gpac.GPacGame(game_map, **kwargs)

    ghost_idx = -1
    player_idx = -1
    player_to_controller = {}
    for player in game.players:
        if 'm' in player:
            if pac_controllers:
                # no ++ iterator in python? Fine, i'll do it the bad way.
                player_to_controller[player] = pac_controllers[player_idx := player_idx + 1]
        else:
            if ghost_controllers:
                player_to_controller[player] = ghost_controllers[ghost_idx := ghost_idx + 1]
    # Game loop, representing one turn.
    while not game.gameover:
        # Evaluate moves for each player.
        for player in game.players:
            ...
            if 'm' in player:
                if pac_controllers is None:
                    ...
                    selected_action_score = -float('inf')
                    for i, s_prime in enumerate(s_primes):
                        value = player_to_controller[player].execute(player, s_prime)
                        if value > selected_action_score:
                            selected_action_score = value
                            selected_action_idx = i
                    player_idx += 1
