...
def write_data_log(filename: str, best_hypervolume: int, pareto_front_genes: List[List], data_log: List[Any]):
...
def load_data_log(filename: str, **kwargs) -> Tuple[float, List[LinearGenotype], List[int], List[float], List[int], List[float], List[float], List[float]]:
    ...
    local_hypervolumes = [float(x[5]) for x in all_data]

    return final_gen_hypervolume, final_pareto_front, evals, max_lengths, mean_lengths, max_widths, mean_widths, local_hypervolumes

def load_average_data_logs(filenames: List[str], config_filename:str) -> Tuple[float, List[LinearGenotype], List[int], List[int], List[float], List[int], List[float], List[float]]:
    ...
    for name in filenames:
        res = load_data_log(name, **config['problem'])
        final_gen_hypervolume, final_pareto_front, evals, max_lengths, mean_lengths, max_widths, mean_widths, local_hypervolumes = res
    ...
    return overall_best_hypervolume, overall_best_pareto_front, x_evals, average_max_lengths, average_mean_lengths, average_max_widths, average_mean_widths, average_local_hypervolumes
