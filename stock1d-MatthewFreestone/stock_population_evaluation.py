...
def multiobjective_population_evaluation(population, yellow=None, **kwargs):
    if not yellow:
        for i in population:
            res = multiobjective_fitness_function(i.genes, **kwargs)
            # Assign member variables based on the evaluation
            i.objectives = [res['length'], res['width']]
            i.visualize = res['visualize']

    else:
        for i in population:
            res = multiobjective_fitness_function(i.genes, shared_edges=True, **kwargs)
            # Assign member variables based on the evaluation
            i.objectives = [res['length'], res['width'], res['edges']]
            i.visualize = res['visualize']
