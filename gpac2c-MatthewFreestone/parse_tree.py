...
class ParseTree:
    ...
    @staticmethod
    def from_string(in_str: str, nonterminals: List[str], terminals: List[str], **kwargs):

        def rec_construct(proc: str):
            # print(in_str)
            depth = 0
            middle = 0
            res = None
            for i,v in enumerate(proc):
                ...
                elif depth == 1 and (v in nonterminals or v == '?'):
                    res = TreeNode(v if v != '?' else 'RAND')
                    middle = i
                    # print(v, i)
                    # print(in_str,'L', in_str[1:middle])
                    # print(in_str,'R',in_str[middle+1:-1])
                    break
            # there's more children
            if res is not None:
                res.left = rec_construct(proc[1:middle])
                res.left.parent = res
                res.right = rec_construct(proc[middle+1:-1])
                res.right.parent = res
            else:
                res = TreeNode(proc[middle])
            return res

        output = __class__(terminals=terminals, nonterminals=nonterminals, **kwargs)
        tokens = in_str.split()
        for i, v in enumerate(tokens):
            # all of my values have a dot, as they're floats
            if '.' in v:
                tokens[i] = float(v)
        output.root = rec_construct(tokens)
        return output
